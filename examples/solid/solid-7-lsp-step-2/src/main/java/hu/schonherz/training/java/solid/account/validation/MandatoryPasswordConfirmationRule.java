...
public class MandatoryPasswordConfirmationRule implements
    ValidationRule<AccountRegistrationRequest> {

  @Override
  public List<Violation> validate(AccountRegistrationRequest object) {
    Validate.notNull(object);
    return StringUtils.isBlank(object.getPasswordConfirmation()) ? Arrays.asList(new Violation(
        "passwordConfirmation", "Password confirmation must be entered")) : Collections
        .<Violation>emptyList();
  }
