...
import hu.schonherz.training.java.solid.cipher.EncrypterService;
...
public class AccountRegistrationRequestToAccountConverter implements Converter<AccountRegistrationRequest, Account> {
  ...
  public AccountRegistrationRequestToAccountConverter(EncrypterService encrypterService) {
    Validate.notNull(encrypterService);
  ...
  public Account convert(AccountRegistrationRequest source) throws ConversionException {
    try {
      ...
      String encryptedPassword = this.encrypterService.encrypt(source.getPassword());
